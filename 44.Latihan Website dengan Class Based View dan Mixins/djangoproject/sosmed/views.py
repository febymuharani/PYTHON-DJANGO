...
class SosmedListView(SosmedSubList,TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context = {
            'page_title' : 'Social Media Memakai Class Base View',
...
class SosmedFormView(View):
    ...
    def get(self, *args, **kwargs):
        if self.mode == 'update':
            ...
            self.form = akun_form = InstagramForm(initial=data, instance=akun_update)
    ...
    def post(self, *args, **kwargs):
        ...
        if kwargs.__contains__('update_id'):
            akun_update = Instagram.objects.get(id=kwargs['update_id'])
            self.form = akun_form = InstagramForm(self.request.POST,instance=akun_update)
        else:
